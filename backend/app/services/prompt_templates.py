...
class BackstoryPromptTemplate:
    """Template for generating a detailed persona backstory."""
    ...
    def render(
        ...
        input_data = []
        if name: input_data.append(f"Name: {name}")
        if age: input_data.append(f"Age: {age}")
        if gender: input_data.append(f"Gender: {gender}")
        if attitude: input_data.append(f"Preferred Attitude: {attitude}")
        if description: input_data.append(f"Partial Description: {description}")

        input_context = "\n".join(input_data) if input_data else "No specific details provided. Generate a fully unique persona."
        attitude_req = f" (matching the attitude '{attitude}')" if attitude else ""

        return (
            f"Generate a detailed, realistic backstory for a new persona based on the following seeds:\n\n"
            f"{input_context}\n\n"
            f"The backstory MUST be a single coherent paragraph (approx 150-250 words) and include:\n"
            f"1. Background and upbringing: Where they grew up and how it shaped them.\n"
            f"2. Occupation and interests: What they do for a living and what they do for fun.\n"
            f"3. Personality traits and quirks: Specific behaviors that make them feel like a real human.\n"
            f"4. Opinions and worldview: Their general stance on life, society, or work.\n"
            f"5. Communication style: How they interact with others{attitude_req}.\n\n"
            f"Guidelines:\n"
            f"- Avoid AI clichés and 'perfect' characters. Give them flaws and edge.\n"
            f"- If a name/age/gender was provided, you MUST use them.\n"
            f"- If no details were provided, create a compelling and specific character from scratch.\n"
            f"- The output should be JUST the backstory paragraph. No headers, no labels, no introduction."
        )
