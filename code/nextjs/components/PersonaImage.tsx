...
const PersonaImage = ({ name, image, index }: PersonaImageProps) => {
  ...
  const [blueskySkeetSkeeted, setBlueskySkeetSkeeted] =
  ...
  return (
    <div key={index}>
      ...
      <div>
        ...
        {generatedSkeet && (
          <p className="text-center text-green-600 mt-4">{generatedSkeet}</p>
        )}
        {isLoading && (
          <div
            role="status"
            className="max-w-lg mx-auto pt-10 flex justify-center"
          >
        ...
        {generatedSkeet && (
          <button
            ...
            onClick={() => postToBluesky(generatedSkeet, name, image)}
          >
            Post to BlueSky
          </button>
        )}
