...
export default function PersonaProfilePage() {
  ...
  useEffect(() => {
    if (!id) return;
    // Use public endpoint — works for both authenticated and anonymous
  ...
  const handleDelete = async () => {
    if (!persona || !window.confirm(`Delete ${persona.name}?`)) return;
    setDeleting(true);
    try {
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="pt-2 space-y-2">
              ...
              {persona.is_owner && (
                ...
                >
                  {deleting ? "Deleting…" : "Delete this persona"}
                </button>
              )}
            </div>
          </div>
